{
    "child_workstations": [
        {
            "parent_ws_id": 1,
            "data_id": 1,
            "child_ws_id": 2,
            "id": 1,
            "child_ws": {
                "id": 2,
                "data_id": 1,
                "name": "WS1",
                "primary_flag": "NO",
                "category_config": "{}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "parent_ws_id": 1,
            "data_id": 1,
            "child_ws_id": 9,
            "id": 5,
            "child_ws": {
                "id": 9,
                "data_id": 1,
                "name": "DHOOP SECTION",
                "primary_flag": "NO",
                "category_config": "{}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "parent_ws_id": 1,
            "data_id": 1,
            "child_ws_id": 11,
            "id": 7,
            "child_ws": {
                "id": 11,
                "data_id": 1,
                "name": "CONE",
                "primary_flag": "NO",
                "category_config": "{}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "parent_ws_id": 1,
            "data_id": 1,
            "child_ws_id": 12,
            "id": 8,
            "child_ws": {
                "id": 12,
                "data_id": 1,
                "name": "INCENSE",
                "primary_flag": "NO",
                "category_config": "{}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "parent_ws_id": 1,
            "data_id": 1,
            "child_ws_id": 13,
            "id": 9,
            "child_ws": {
                "id": 13,
                "data_id": 1,
                "name": "FESTIVAL INCENSE",
                "primary_flag": "NO",
                "category_config": "{}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "parent_ws_id": 1,
            "data_id": 1,
            "child_ws_id": 14,
            "id": 10,
            "child_ws": {
                "id": 14,
                "data_id": 1,
                "name": "BACKFLOW CONE DEPARTMENT",
                "primary_flag": "NO",
                "category_config": "{}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        }
    ],
    "workstation_jobs": [
        {
            "date_allot": "2024-06-01",
            "workstation_id": 1,
            "data_id": 1,
            "id": 147,
            "inventory_id": null,
            "item_id": 6,
            "wip_inventory_id": null,
            "qty_allot": 10.0,
            "reject_inventory_id": null,
            "qty_recv": 0.0,
            "qty_wip": 0.0,
            "qty_reject": 0.0,
            "item": {
                "rate": 4.0,
                "regdate": "2024-01-04",
                "raw_flag": "YES",
                "data_id": 1,
                "code": "9345",
                "id": 6,
                "name": "Vedant product",
                "unit": "dz"
            },
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "date_allot": "2024-06-01",
            "workstation_id": 1,
            "data_id": 1,
            "id": 125,
            "inventory_id": null,
            "item_id": 50,
            "wip_inventory_id": null,
            "qty_allot": 0.0,
            "reject_inventory_id": null,
            "qty_recv": 0.0,
            "qty_wip": 0.0,
            "qty_reject": 0.0,
            "item": {
                "rate": 23.0,
                "regdate": "2024-05-25",
                "raw_flag": "NO",
                "data_id": 1,
                "code": "na",
                "id": 50,
                "name": "jaygodara45",
                "unit": "46"
            },
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "date_allot": "2024-06-01",
            "workstation_id": 1,
            "data_id": 1,
            "id": 152,
            "inventory_id": null,
            "item_id": 2,
            "wip_inventory_id": null,
            "qty_allot": 0.0,
            "reject_inventory_id": null,
            "qty_recv": 0.0,
            "qty_wip": 0.0,
            "qty_reject": 0.0,
            "item": {
                "rate": 12.0,
                "regdate": "2023-11-18",
                "raw_flag": "NO",
                "data_id": 1,
                "code": "FG001",
                "id": 2,
                "name": "FG Item 1 bs",
                "unit": "dozend"
            },
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "date_allot": "2024-06-01",
            "workstation_id": 1,
            "data_id": 1,
            "id": 140,
            "inventory_id": null,
            "item_id": 19,
            "wip_inventory_id": null,
            "qty_allot": 59.0,
            "reject_inventory_id": null,
            "qty_recv": 0.0,
            "qty_wip": 0.0,
            "qty_reject": 0.0,
            "item": {
                "rate": 231.0,
                "regdate": "2024-02-05",
                "raw_flag": "NO",
                "data_id": 1,
                "code": "CP002E",
                "id": 19,
                "name": "COAL POWDERR",
                "unit": "KGG"
            },
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "date_allot": "2024-06-01",
            "workstation_id": 1,
            "data_id": 1,
            "id": 120,
            "inventory_id": null,
            "item_id": 48,
            "wip_inventory_id": null,
            "qty_allot": 1345.0,
            "reject_inventory_id": null,
            "qty_recv": 0.0,
            "qty_wip": 0.0,
            "qty_reject": 0.0,
            "item": {
                "rate": 10.0,
                "regdate": "2024-05-25",
                "raw_flag": "NO",
                "data_id": 1,
                "code": "IM1.1",
                "id": 48,
                "name": "Item 1.1",
                "unit": "pcs"
            },
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "date_allot": "2024-06-01",
            "workstation_id": 1,
            "data_id": 1,
            "id": 156,
            "inventory_id": null,
            "item_id": 12,
            "wip_inventory_id": null,
            "qty_allot": 8.0,
            "reject_inventory_id": null,
            "qty_recv": 0.0,
            "qty_wip": 0.0,
            "qty_reject": 0.0,
            "item": {
                "rate": 12.0,
                "regdate": "2024-01-04",
                "raw_flag": "YES",
                "data_id": 1,
                "code": "9350",
                "id": 12,
                "name": "Vedant HsssE Item5",
                "unit": "dz"
            },
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "date_allot": "2024-06-01",
            "workstation_id": 1,
            "data_id": 1,
            "id": 142,
            "inventory_id": null,
            "item_id": 18,
            "wip_inventory_id": null,
            "qty_allot": 40.0,
            "reject_inventory_id": null,
            "qty_recv": 0.0,
            "qty_wip": 0.0,
            "qty_reject": 0.0,
            "item": {
                "rate": 0.0,
                "regdate": "2024-02-05",
                "raw_flag": "NO",
                "data_id": 1,
                "code": "JP0011",
                "id": 18,
                "name": "JOSH POWDER",
                "unit": "KG"
            },
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "date_allot": "2024-06-01",
            "workstation_id": 1,
            "data_id": 1,
            "id": 150,
            "inventory_id": null,
            "item_id": 68,
            "wip_inventory_id": null,
            "qty_allot": 0.0,
            "reject_inventory_id": null,
            "qty_recv": 0.0,
            "qty_wip": 0.0,
            "qty_reject": 0.0,
            "item": {
                "rate": 45.0,
                "regdate": "2024-05-31",
                "raw_flag": "NO",
                "data_id": 1,
                "code": "jjnjsd",
                "id": 68,
                "name": "hey",
                "unit": "kg"
            },
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        },
        {
            "date_allot": "2024-06-01",
            "workstation_id": 1,
            "data_id": 1,
            "id": 144,
            "inventory_id": null,
            "item_id": 16,
            "wip_inventory_id": null,
            "qty_allot": 0.0,
            "reject_inventory_id": null,
            "qty_recv": 0.0,
            "qty_wip": 0.0,
            "qty_reject": 0.0,
            "item": {
                "rate": 5.0,
                "regdate": "2024-02-05",
                "raw_flag": "NO",
                "data_id": 1,
                "code": "NA",
                "id": 16,
                "name": "DHOP 1.1 INCH BUNDLED",
                "unit": "KGS"
            },
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            }
        }
    ],
    "workstation_resources": [],
    "workstation": {
        "data_id": 1,
        "id": 1,
        "primary_flag": "YES",
        "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
        "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
        "database": {
            "id": 1,
            "name": "VEDANT AGRAWAL_J3VU4NZYT5",
            "key": "YLKS04NS2W",
            "regdate": "2023-11-18",
            "status": "Active"
        }
    },
    "data": {
        "2": {
            "date": "2024-06-01",
            "note": "WS1",
            "chart_items": [
                [
                    151,
                    "FG Item 1 bs",
                    6.0,
                    "dozend",
                    0,
                    2
                ],
                [
                    154,
                    "Item 1.1",
                    255.0,
                    "pcs",
                    0,
                    48
                ],
                [
                    129,
                    "Item 1.1",
                    255.0,
                    "pcs",
                    0,
                    48
                ],
                [
                    157,
                    "hey",
                    30.0,
                    "kg",
                    0,
                    68
                ]
            ],
            "ws_jobs": [
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 2,
                    "data_id": 1,
                    "id": 151,
                    "inventory_id": 519,
                    "item_id": 2,
                    "wip_inventory_id": null,
                    "qty_allot": 6.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 2,
                    "data_id": 1,
                    "id": 154,
                    "inventory_id": 527,
                    "item_id": 48,
                    "wip_inventory_id": null,
                    "qty_allot": 4.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 2,
                    "data_id": 1,
                    "id": 129,
                    "inventory_id": 477,
                    "item_id": 48,
                    "wip_inventory_id": 479,
                    "qty_allot": 251.0,
                    "reject_inventory_id": 480,
                    "qty_recv": 12.0,
                    "qty_wip": 23.0,
                    "qty_reject": 1.0
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 2,
                    "data_id": 1,
                    "id": 157,
                    "inventory_id": 529,
                    "item_id": 68,
                    "wip_inventory_id": null,
                    "qty_allot": 15.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0
                }
            ],
            "chart_resources": [
                {
                    "id": 86,
                    "resource_id": 1,
                    "time_lost": 0.0,
                    "contract_mode": "PAYROLL",
                    "time_allot": 11.0,
                    "workstation_id": 2,
                    "date_allot": "2024-06-01",
                    "data_id": 1
                },
                {
                    "id": 89,
                    "resource_id": 5,
                    "time_lost": 0.0,
                    "contract_mode": "PAYROLL",
                    "time_allot": 4.0,
                    "workstation_id": 2,
                    "date_allot": "2024-06-01",
                    "data_id": 1
                },
                {
                    "id": 90,
                    "resource_id": 3,
                    "time_lost": 0.0,
                    "contract_mode": "contract",
                    "time_allot": 8.0,
                    "workstation_id": 2,
                    "date_allot": "2024-06-01",
                    "data_id": 1
                }
            ]
        },
        "9": {
            "date": "2024-06-01",
            "note": "DHOOP SECTION",
            "chart_items": [],
            "ws_jobs": [],
            "chart_resources": []
        },
        "11": {
            "date": "2024-06-01",
            "note": "CONE",
            "chart_items": [
                [
                    160,
                    "FG Item 1 bs",
                    25.0,
                    "dozend",
                    0,
                    2
                ]
            ],
            "ws_jobs": [
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 11,
                    "data_id": 1,
                    "id": 160,
                    "inventory_id": 542,
                    "item_id": 2,
                    "wip_inventory_id": null,
                    "qty_allot": 25.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0
                }
            ],
            "chart_resources": []
        },
        "12": {
            "date": "2024-06-01",
            "note": "INCENSE",
            "chart_items": [],
            "ws_jobs": [],
            "chart_resources": []
        },
        "13": {
            "date": "2024-06-01",
            "note": "FESTIVAL INCENSE",
            "chart_items": [],
            "ws_jobs": [],
            "chart_resources": []
        },
        "14": {
            "date": "2024-06-01",
            "note": "BACKFLOW CONE DEPARTMENT",
            "chart_items": [],
            "ws_jobs": [],
            "chart_resources": []
        }
    },
    "WS_DATE": "2024-06-01",
    "leaf_data": {
        "jobs": {},
        "materials": {}
    },
    "jobs_breakup": {
        "1": {
            "workstation": {
                "data_id": 1,
                "id": 1,
                "primary_flag": "YES",
                "name": "VEDANT AGRAWAL_J3VU4NZYT5_primary_ws",
                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [1]}}",
                "database": {
                    "id": 1,
                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                    "key": "YLKS04NS2W",
                    "regdate": "2023-11-18",
                    "status": "Active"
                }
            },
            "totals": {
                "6": 10.0,
                "50": 0.0,
                "2": 31.0,
                "19": 59.0,
                "48": 1600.0,
                "12": 8.0,
                "18": 40.0,
                "68": 30.0,
                "16": 0.0
            },
            "recv_totals": {
                "6": 0,
                "50": 0,
                "2": 0,
                "19": 0,
                "48": 12.0,
                "12": 0,
                "18": 0,
                "68": 0,
                "16": 0
            },
            "wip_totals": {
                "6": 0.0,
                "50": 0.0,
                "2": 0.0,
                "19": 0.0,
                "48": 23.0,
                "12": 0.0,
                "18": 0.0,
                "68": 0.0,
                "16": 0.0
            },
            "reject_totals": {
                "6": 0.0,
                "50": 0.0,
                "2": 0.0,
                "19": 0.0,
                "48": 1.0,
                "12": 0.0,
                "18": 0.0,
                "68": 0.0,
                "16": 0.0
            },
            "material_issues": [
                {
                    "workstation_id": 1,
                    "item_id": 50,
                    "issue_qty": 0.0,
                    "return_qty": 0.0,
                    "date_issue": "2024-06-01",
                    "inventory_id": 494,
                    "wip_flag": "YES",
                    "item_unit": "46",
                    "id": 89,
                    "reject_qty": 0.0,
                    "data_id": 1,
                    "reject_inventory_id": null,
                    "item": {
                        "rate": 23.0,
                        "regdate": "2024-05-25",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "na",
                        "id": 50,
                        "name": "jaygodara45",
                        "unit": "46"
                    }
                },
                {
                    "workstation_id": 1,
                    "item_id": 48,
                    "issue_qty": 0.0,
                    "return_qty": 0.0,
                    "date_issue": "2024-06-01",
                    "inventory_id": 496,
                    "wip_flag": "YES",
                    "item_unit": "dxy",
                    "id": 91,
                    "reject_qty": 0.0,
                    "data_id": 1,
                    "reject_inventory_id": null,
                    "item": {
                        "rate": 10.0,
                        "regdate": "2024-05-25",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "IM1.1",
                        "id": 48,
                        "name": "Item 1.1",
                        "unit": "pcs"
                    }
                },
                {
                    "workstation_id": 1,
                    "item_id": 51,
                    "issue_qty": 0.0,
                    "return_qty": 0.0,
                    "date_issue": "2024-06-01",
                    "inventory_id": 501,
                    "wip_flag": "NO",
                    "item_unit": "dxs",
                    "id": 94,
                    "reject_qty": 0.0,
                    "data_id": 1,
                    "reject_inventory_id": null,
                    "item": {
                        "rate": 0.0,
                        "regdate": "2024-05-25",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "abc",
                        "id": 51,
                        "name": "Jay ka item 6",
                        "unit": "dxs"
                    }
                },
                {
                    "workstation_id": 1,
                    "item_id": 17,
                    "issue_qty": 0.0,
                    "return_qty": 0.0,
                    "date_issue": "2024-06-01",
                    "inventory_id": 504,
                    "wip_flag": "NO",
                    "item_unit": "KGS",
                    "id": 96,
                    "reject_qty": 0.0,
                    "data_id": 1,
                    "reject_inventory_id": null,
                    "item": {
                        "rate": 5.0,
                        "regdate": "2024-02-05",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "NA",
                        "id": 17,
                        "name": "DHOOPBatti 1.1 INCH PRODUCED",
                        "unit": "KGS"
                    }
                },
                {
                    "workstation_id": 1,
                    "item_id": 4,
                    "issue_qty": 0.0,
                    "return_qty": 0.0,
                    "date_issue": "2024-06-01",
                    "inventory_id": 523,
                    "wip_flag": "NO",
                    "item_unit": "kgs",
                    "id": 102,
                    "reject_qty": 0.0,
                    "data_id": 1,
                    "reject_inventory_id": null,
                    "item": {
                        "rate": 11.0,
                        "regdate": "2023-11-20",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "rm1",
                        "id": 4,
                        "name": "rm item 1",
                        "unit": "kgs"
                    }
                },
                {
                    "workstation_id": 1,
                    "item_id": 3,
                    "issue_qty": 0.0,
                    "return_qty": 0.0,
                    "date_issue": "2024-06-01",
                    "inventory_id": 524,
                    "wip_flag": "NO",
                    "item_unit": "pcs",
                    "id": 103,
                    "reject_qty": 0.0,
                    "data_id": 1,
                    "reject_inventory_id": null,
                    "item": {
                        "rate": 0.0,
                        "regdate": "2023-11-20",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "sf1",
                        "id": 3,
                        "name": "Sabka Product",
                        "unit": "pcs"
                    }
                },
                {
                    "workstation_id": 1,
                    "item_id": 2,
                    "issue_qty": 0.0,
                    "return_qty": 0.0,
                    "date_issue": "2024-06-01",
                    "inventory_id": 525,
                    "wip_flag": "YES",
                    "item_unit": "dozend",
                    "id": 104,
                    "reject_qty": 0.0,
                    "data_id": 1,
                    "reject_inventory_id": null,
                    "item": {
                        "rate": 12.0,
                        "regdate": "2023-11-18",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "FG001",
                        "id": 2,
                        "name": "FG Item 1 bs",
                        "unit": "dozend"
                    }
                }
            ],
            "material_estimates": {
                "4": 0.0,
                "3": 0.0,
                "51": 1616.0,
                "17": 0.0,
                "50": 0.0,
                "48": 1345.0,
                "2": 0.0
            },
            "material_estimate_totals": {
                "4": 16.275,
                "3": 390.6,
                "51": 1616.0,
                "17": 0.0,
                "50": 0.0,
                "48": 1600.0,
                "2": 31.0
            },
            "material_estimate_totals_from_recv": {
                "4": 0.0,
                "3": 0.0,
                "51": 0.0,
                "17": 0.0,
                "50": 0.0,
                "48": 1600.0,
                "2": 31.0
            },
            "material_issue_totals": {
                "50": 0.0,
                "48": 0.0,
                "51": 0.0,
                "17": 0.0,
                "4": 0.0,
                "3": 0.0,
                "2": 0.0
            },
            "material_return_totals": {
                "50": 0.0,
                "48": 0.0,
                "51": 0.0,
                "17": 0.0,
                "4": 0.0,
                "3": 0.0,
                "2": 0.0
            },
            "material_reject_totals": {
                "50": 0.0,
                "48": 0.0,
                "51": 0.0,
                "17": 0.0,
                "4": 0.0,
                "3": 0.0,
                "2": 0.0
            },
            "job_work_totals": 31487.0,
            "capacity_totals": 1410.0,
            "ws_date": "2024-06-01",
            "jobs": [
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 1,
                    "data_id": 1,
                    "id": 147,
                    "inventory_id": null,
                    "item_id": 6,
                    "wip_inventory_id": null,
                    "qty_allot": 10.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0,
                    "item": {
                        "rate": 4.0,
                        "regdate": "2024-01-04",
                        "raw_flag": "YES",
                        "data_id": 1,
                        "code": "9345",
                        "id": 6,
                        "name": "Vedant product",
                        "unit": "dz"
                    }
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 1,
                    "data_id": 1,
                    "id": 125,
                    "inventory_id": null,
                    "item_id": 50,
                    "wip_inventory_id": null,
                    "qty_allot": 0.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0,
                    "item": {
                        "rate": 23.0,
                        "regdate": "2024-05-25",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "na",
                        "id": 50,
                        "name": "jaygodara45",
                        "unit": "46"
                    }
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 1,
                    "data_id": 1,
                    "id": 152,
                    "inventory_id": null,
                    "item_id": 2,
                    "wip_inventory_id": null,
                    "qty_allot": 0.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0,
                    "item": {
                        "rate": 12.0,
                        "regdate": "2023-11-18",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "FG001",
                        "id": 2,
                        "name": "FG Item 1 bs",
                        "unit": "dozend"
                    }
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 1,
                    "data_id": 1,
                    "id": 140,
                    "inventory_id": null,
                    "item_id": 19,
                    "wip_inventory_id": null,
                    "qty_allot": 59.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0,
                    "item": {
                        "rate": 231.0,
                        "regdate": "2024-02-05",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "CP002E",
                        "id": 19,
                        "name": "COAL POWDERR",
                        "unit": "KGG"
                    }
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 1,
                    "data_id": 1,
                    "id": 120,
                    "inventory_id": null,
                    "item_id": 48,
                    "wip_inventory_id": null,
                    "qty_allot": 1345.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0,
                    "item": {
                        "rate": 10.0,
                        "regdate": "2024-05-25",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "IM1.1",
                        "id": 48,
                        "name": "Item 1.1",
                        "unit": "pcs"
                    }
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 1,
                    "data_id": 1,
                    "id": 156,
                    "inventory_id": null,
                    "item_id": 12,
                    "wip_inventory_id": null,
                    "qty_allot": 8.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0,
                    "item": {
                        "rate": 12.0,
                        "regdate": "2024-01-04",
                        "raw_flag": "YES",
                        "data_id": 1,
                        "code": "9350",
                        "id": 12,
                        "name": "Vedant HsssE Item5",
                        "unit": "dz"
                    }
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 1,
                    "data_id": 1,
                    "id": 142,
                    "inventory_id": null,
                    "item_id": 18,
                    "wip_inventory_id": null,
                    "qty_allot": 40.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0,
                    "item": {
                        "rate": 0.0,
                        "regdate": "2024-02-05",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "JP0011",
                        "id": 18,
                        "name": "JOSH POWDER",
                        "unit": "KG"
                    }
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 1,
                    "data_id": 1,
                    "id": 150,
                    "inventory_id": null,
                    "item_id": 68,
                    "wip_inventory_id": null,
                    "qty_allot": 0.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0,
                    "item": {
                        "rate": 45.0,
                        "regdate": "2024-05-31",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "jjnjsd",
                        "id": 68,
                        "name": "hey",
                        "unit": "kg"
                    }
                },
                {
                    "date_allot": "2024-06-01",
                    "workstation_id": 1,
                    "data_id": 1,
                    "id": 144,
                    "inventory_id": null,
                    "item_id": 16,
                    "wip_inventory_id": null,
                    "qty_allot": 0.0,
                    "reject_inventory_id": null,
                    "qty_recv": 0.0,
                    "qty_wip": 0.0,
                    "qty_reject": 0.0,
                    "item": {
                        "rate": 5.0,
                        "regdate": "2024-02-05",
                        "raw_flag": "NO",
                        "data_id": 1,
                        "code": "NA",
                        "id": 16,
                        "name": "DHOP 1.1 INCH BUNDLED",
                        "unit": "KGS"
                    }
                }
            ],
            "resources": [
                {
                    "id": 86,
                    "resource_id": 1,
                    "time_lost": 0.0,
                    "contract_mode": "PAYROLL",
                    "time_allot": 11.0,
                    "workstation_id": 2,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "NA",
                        "name": "Labor 11",
                        "salary": 50.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 1,
                        "gender": "Male",
                        "regdate": "2023-11-19",
                        "data_id": 1
                    }
                },
                {
                    "id": 89,
                    "resource_id": 5,
                    "time_lost": 0.0,
                    "contract_mode": "PAYROLL",
                    "time_allot": 4.0,
                    "workstation_id": 2,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "PW001",
                        "name": "SALIM",
                        "salary": 45.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 5,
                        "gender": "WORKER",
                        "regdate": "2024-02-05",
                        "data_id": 1
                    }
                },
                {
                    "id": 90,
                    "resource_id": 3,
                    "time_lost": 0.0,
                    "contract_mode": "contract",
                    "time_allot": 8.0,
                    "workstation_id": 2,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "LA04",
                        "name": "LABOR 2",
                        "salary": 45.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 3,
                        "gender": "WORKER",
                        "regdate": "2024-02-04",
                        "data_id": 1
                    }
                },
                {
                    "id": 58,
                    "resource_id": 2,
                    "time_lost": 0.0,
                    "contract_mode": "PAYROLL",
                    "time_allot": 8.0,
                    "workstation_id": 10,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "NA",
                        "name": "labor 2",
                        "salary": 20.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 2,
                        "gender": "Male",
                        "regdate": "2023-11-21",
                        "data_id": 1
                    }
                },
                {
                    "id": 60,
                    "resource_id": 2,
                    "time_lost": 0.0,
                    "contract_mode": "CONTRACT",
                    "time_allot": 8.0,
                    "workstation_id": 15,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "NA",
                        "name": "labor 2",
                        "salary": 20.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 2,
                        "gender": "Male",
                        "regdate": "2023-11-21",
                        "data_id": 1
                    }
                }
            ],
            "child_resources": [
                {
                    "id": 86,
                    "resource_id": 1,
                    "time_lost": 0.0,
                    "contract_mode": "PAYROLL",
                    "time_allot": 11.0,
                    "workstation_id": 2,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "NA",
                        "name": "Labor 11",
                        "salary": 50.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 1,
                        "gender": "Male",
                        "regdate": "2023-11-19",
                        "data_id": 1
                    }
                },
                {
                    "id": 89,
                    "resource_id": 5,
                    "time_lost": 0.0,
                    "contract_mode": "PAYROLL",
                    "time_allot": 4.0,
                    "workstation_id": 2,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "PW001",
                        "name": "SALIM",
                        "salary": 45.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 5,
                        "gender": "WORKER",
                        "regdate": "2024-02-05",
                        "data_id": 1
                    }
                },
                {
                    "id": 90,
                    "resource_id": 3,
                    "time_lost": 0.0,
                    "contract_mode": "contract",
                    "time_allot": 8.0,
                    "workstation_id": 2,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "LA04",
                        "name": "LABOR 2",
                        "salary": 45.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 3,
                        "gender": "WORKER",
                        "regdate": "2024-02-04",
                        "data_id": 1
                    }
                },
                {
                    "id": 58,
                    "resource_id": 2,
                    "time_lost": 0.0,
                    "contract_mode": "PAYROLL",
                    "time_allot": 8.0,
                    "workstation_id": 10,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "NA",
                        "name": "labor 2",
                        "salary": 20.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 2,
                        "gender": "Male",
                        "regdate": "2023-11-21",
                        "data_id": 1
                    }
                },
                {
                    "id": 60,
                    "resource_id": 2,
                    "time_lost": 0.0,
                    "contract_mode": "CONTRACT",
                    "time_allot": 8.0,
                    "workstation_id": 15,
                    "date_allot": "2024-06-01",
                    "data_id": 1,
                    "resource": {
                        "code": "NA",
                        "name": "labor 2",
                        "salary": 20.0,
                        "status": "Active",
                        "contract_mode": "PAYROLL",
                        "id": 2,
                        "gender": "Male",
                        "regdate": "2023-11-21",
                        "data_id": 1
                    }
                }
            ],
            "category_config": {
                "item_categories": {
                    "filter_type": "inclusive",
                    "filters_array": [
                        1
                    ]
                }
            },
            "childs": {
                "2": {
                    "workstation": {
                        "id": 2,
                        "data_id": 1,
                        "name": "WS1",
                        "primary_flag": "NO",
                        "category_config": "{}",
                        "database": {
                            "id": 1,
                            "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                            "key": "YLKS04NS2W",
                            "regdate": "2023-11-18",
                            "status": "Active"
                        }
                    },
                    "totals": {
                        "2": 6.0,
                        "48": 255.0,
                        "68": 30.0
                    },
                    "recv_totals": {
                        "2": 0,
                        "48": 12.0,
                        "68": 0
                    },
                    "wip_totals": {
                        "2": 0.0,
                        "48": 23.0,
                        "68": 0.0
                    },
                    "reject_totals": {
                        "2": 0.0,
                        "48": 1.0,
                        "68": 0.0
                    },
                    "material_issues": [
                        {
                            "workstation_id": 2,
                            "item_id": 48,
                            "issue_qty": 0.0,
                            "return_qty": 0.0,
                            "date_issue": "2024-06-01",
                            "inventory_id": 495,
                            "wip_flag": "YES",
                            "item_unit": "dxy",
                            "id": 90,
                            "reject_qty": 0.0,
                            "data_id": 1,
                            "reject_inventory_id": null,
                            "item": {
                                "rate": 10.0,
                                "regdate": "2024-05-25",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "IM1.1",
                                "id": 48,
                                "name": "Item 1.1",
                                "unit": "pcs"
                            }
                        },
                        {
                            "workstation_id": 2,
                            "item_id": 4,
                            "issue_qty": 0.0,
                            "return_qty": 0.0,
                            "date_issue": "2024-06-01",
                            "inventory_id": 520,
                            "wip_flag": "NO",
                            "item_unit": "kgs",
                            "id": 99,
                            "reject_qty": 0.0,
                            "data_id": 1,
                            "reject_inventory_id": null,
                            "item": {
                                "rate": 11.0,
                                "regdate": "2023-11-20",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "rm1",
                                "id": 4,
                                "name": "rm item 1",
                                "unit": "kgs"
                            }
                        },
                        {
                            "workstation_id": 2,
                            "item_id": 3,
                            "issue_qty": 0.0,
                            "return_qty": 0.0,
                            "date_issue": "2024-06-01",
                            "inventory_id": 521,
                            "wip_flag": "NO",
                            "item_unit": "pcs",
                            "id": 100,
                            "reject_qty": 0.0,
                            "data_id": 1,
                            "reject_inventory_id": null,
                            "item": {
                                "rate": 0.0,
                                "regdate": "2023-11-20",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "sf1",
                                "id": 3,
                                "name": "Sabka Product",
                                "unit": "pcs"
                            }
                        },
                        {
                            "workstation_id": 2,
                            "item_id": 2,
                            "issue_qty": 0.0,
                            "return_qty": 0.0,
                            "date_issue": "2024-06-01",
                            "inventory_id": 522,
                            "wip_flag": "YES",
                            "item_unit": "dozend",
                            "id": 101,
                            "reject_qty": 0.0,
                            "data_id": 1,
                            "reject_inventory_id": null,
                            "item": {
                                "rate": 12.0,
                                "regdate": "2023-11-18",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "FG001",
                                "id": 2,
                                "name": "FG Item 1 bs",
                                "unit": "dozend"
                            }
                        }
                    ],
                    "material_estimates": {
                        "4": 3.1500000000000004,
                        "3": 75.60000000000001,
                        "48": 255.0,
                        "2": 6.0
                    },
                    "material_estimate_totals": {
                        "4": 3.1500000000000004,
                        "3": 75.60000000000001,
                        "48": 255.0,
                        "2": 6.0
                    },
                    "material_estimate_totals_from_recv": {
                        "4": 0.0,
                        "3": 0.0,
                        "48": 255.0,
                        "2": 6.0
                    },
                    "material_issue_totals": {
                        "48": 0.0,
                        "4": 0.0,
                        "3": 0.0,
                        "2": 0.0
                    },
                    "material_return_totals": {
                        "48": 0.0,
                        "4": 0.0,
                        "3": 0.0,
                        "2": 0.0
                    },
                    "material_reject_totals": {
                        "48": 0.0,
                        "4": 0.0,
                        "3": 0.0,
                        "2": 0.0
                    },
                    "job_work_totals": 3972.0,
                    "capacity_totals": 1090.0,
                    "ws_date": "2024-06-01",
                    "jobs": [
                        {
                            "date_allot": "2024-06-01",
                            "workstation_id": 2,
                            "data_id": 1,
                            "id": 151,
                            "inventory_id": 519,
                            "item_id": 2,
                            "wip_inventory_id": null,
                            "qty_allot": 6.0,
                            "reject_inventory_id": null,
                            "qty_recv": 0.0,
                            "qty_wip": 0.0,
                            "qty_reject": 0.0,
                            "item": {
                                "rate": 12.0,
                                "regdate": "2023-11-18",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "FG001",
                                "id": 2,
                                "name": "FG Item 1 bs",
                                "unit": "dozend"
                            }
                        },
                        {
                            "date_allot": "2024-06-01",
                            "workstation_id": 2,
                            "data_id": 1,
                            "id": 154,
                            "inventory_id": 527,
                            "item_id": 48,
                            "wip_inventory_id": null,
                            "qty_allot": 4.0,
                            "reject_inventory_id": null,
                            "qty_recv": 0.0,
                            "qty_wip": 0.0,
                            "qty_reject": 0.0,
                            "item": {
                                "rate": 10.0,
                                "regdate": "2024-05-25",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "IM1.1",
                                "id": 48,
                                "name": "Item 1.1",
                                "unit": "pcs"
                            }
                        },
                        {
                            "date_allot": "2024-06-01",
                            "workstation_id": 2,
                            "data_id": 1,
                            "id": 129,
                            "inventory_id": 477,
                            "item_id": 48,
                            "wip_inventory_id": 479,
                            "qty_allot": 251.0,
                            "reject_inventory_id": 480,
                            "qty_recv": 12.0,
                            "qty_wip": 23.0,
                            "qty_reject": 1.0,
                            "item": {
                                "rate": 10.0,
                                "regdate": "2024-05-25",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "IM1.1",
                                "id": 48,
                                "name": "Item 1.1",
                                "unit": "pcs"
                            }
                        },
                        {
                            "date_allot": "2024-06-01",
                            "workstation_id": 2,
                            "data_id": 1,
                            "id": 157,
                            "inventory_id": 529,
                            "item_id": 68,
                            "wip_inventory_id": null,
                            "qty_allot": 15.0,
                            "reject_inventory_id": null,
                            "qty_recv": 0.0,
                            "qty_wip": 0.0,
                            "qty_reject": 0.0,
                            "item": {
                                "rate": 45.0,
                                "regdate": "2024-05-31",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "jjnjsd",
                                "id": 68,
                                "name": "hey",
                                "unit": "kg"
                            }
                        }
                    ],
                    "resources": [
                        {
                            "id": 86,
                            "resource_id": 1,
                            "time_lost": 0.0,
                            "contract_mode": "PAYROLL",
                            "time_allot": 11.0,
                            "workstation_id": 2,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "NA",
                                "name": "Labor 11",
                                "salary": 50.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 1,
                                "gender": "Male",
                                "regdate": "2023-11-19",
                                "data_id": 1
                            }
                        },
                        {
                            "id": 89,
                            "resource_id": 5,
                            "time_lost": 0.0,
                            "contract_mode": "PAYROLL",
                            "time_allot": 4.0,
                            "workstation_id": 2,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "PW001",
                                "name": "SALIM",
                                "salary": 45.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 5,
                                "gender": "WORKER",
                                "regdate": "2024-02-05",
                                "data_id": 1
                            }
                        },
                        {
                            "id": 90,
                            "resource_id": 3,
                            "time_lost": 0.0,
                            "contract_mode": "contract",
                            "time_allot": 8.0,
                            "workstation_id": 2,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "LA04",
                                "name": "LABOR 2",
                                "salary": 45.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 3,
                                "gender": "WORKER",
                                "regdate": "2024-02-04",
                                "data_id": 1
                            }
                        }
                    ],
                    "child_resources": [
                        {
                            "id": 86,
                            "resource_id": 1,
                            "time_lost": 0.0,
                            "contract_mode": "PAYROLL",
                            "time_allot": 11.0,
                            "workstation_id": 2,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "NA",
                                "name": "Labor 11",
                                "salary": 50.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 1,
                                "gender": "Male",
                                "regdate": "2023-11-19",
                                "data_id": 1
                            }
                        },
                        {
                            "id": 89,
                            "resource_id": 5,
                            "time_lost": 0.0,
                            "contract_mode": "PAYROLL",
                            "time_allot": 4.0,
                            "workstation_id": 2,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "PW001",
                                "name": "SALIM",
                                "salary": 45.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 5,
                                "gender": "WORKER",
                                "regdate": "2024-02-05",
                                "data_id": 1
                            }
                        },
                        {
                            "id": 90,
                            "resource_id": 3,
                            "time_lost": 0.0,
                            "contract_mode": "contract",
                            "time_allot": 8.0,
                            "workstation_id": 2,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "LA04",
                                "name": "LABOR 2",
                                "salary": 45.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 3,
                                "gender": "WORKER",
                                "regdate": "2024-02-04",
                                "data_id": 1
                            }
                        }
                    ],
                    "category_config": {},
                    "childs": {
                        "6": {
                            "workstation": {
                                "id": 6,
                                "data_id": 1,
                                "name": "WS1.1",
                                "primary_flag": "NO",
                                "category_config": "{\"item_categories\": {\"filter_type\": \"inclusive\", \"filters_array\": [\"2\"]}}",
                                "database": {
                                    "id": 1,
                                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                                    "key": "YLKS04NS2W",
                                    "regdate": "2023-11-18",
                                    "status": "Active"
                                }
                            },
                            "totals": {
                                "68": 10.0
                            },
                            "recv_totals": {
                                "68": 0
                            },
                            "wip_totals": {
                                "68": 0.0
                            },
                            "reject_totals": {
                                "68": 0.0
                            },
                            "material_issues": [],
                            "material_estimates": {},
                            "material_estimate_totals": {},
                            "material_estimate_totals_from_recv": {},
                            "material_issue_totals": {},
                            "material_return_totals": {},
                            "material_reject_totals": {},
                            "job_work_totals": 450.0,
                            "capacity_totals": 0,
                            "ws_date": "2024-06-01",
                            "jobs": [
                                {
                                    "date_allot": "2024-06-01",
                                    "workstation_id": 6,
                                    "data_id": 1,
                                    "id": 158,
                                    "inventory_id": 530,
                                    "item_id": 68,
                                    "wip_inventory_id": null,
                                    "qty_allot": 10.0,
                                    "reject_inventory_id": null,
                                    "qty_recv": 0.0,
                                    "qty_wip": 0.0,
                                    "qty_reject": 0.0,
                                    "item": {
                                        "rate": 45.0,
                                        "regdate": "2024-05-31",
                                        "raw_flag": "NO",
                                        "data_id": 1,
                                        "code": "jjnjsd",
                                        "id": 68,
                                        "name": "hey",
                                        "unit": "kg"
                                    }
                                }
                            ],
                            "resources": [],
                            "child_resources": [],
                            "category_config": {
                                "item_categories": {
                                    "filter_type": "inclusive",
                                    "filters_array": [
                                        "2"
                                    ]
                                }
                            },
                            "childs": {}
                        },
                        "7": {
                            "workstation": {
                                "id": 7,
                                "data_id": 1,
                                "name": "WS1.2",
                                "primary_flag": "NO",
                                "category_config": "{}",
                                "database": {
                                    "id": 1,
                                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                                    "key": "YLKS04NS2W",
                                    "regdate": "2023-11-18",
                                    "status": "Active"
                                }
                            },
                            "totals": {
                                "68": 5.0
                            },
                            "recv_totals": {
                                "68": 0
                            },
                            "wip_totals": {
                                "68": 0.0
                            },
                            "reject_totals": {
                                "68": 0.0
                            },
                            "material_issues": [],
                            "material_estimates": {},
                            "material_estimate_totals": {},
                            "material_estimate_totals_from_recv": {},
                            "material_issue_totals": {},
                            "material_return_totals": {},
                            "material_reject_totals": {},
                            "job_work_totals": 225.0,
                            "capacity_totals": 0,
                            "ws_date": "2024-06-01",
                            "jobs": [
                                {
                                    "date_allot": "2024-06-01",
                                    "workstation_id": 7,
                                    "data_id": 1,
                                    "id": 159,
                                    "inventory_id": 531,
                                    "item_id": 68,
                                    "wip_inventory_id": null,
                                    "qty_allot": 5.0,
                                    "reject_inventory_id": null,
                                    "qty_recv": 0.0,
                                    "qty_wip": 0.0,
                                    "qty_reject": 0.0,
                                    "item": {
                                        "rate": 45.0,
                                        "regdate": "2024-05-31",
                                        "raw_flag": "NO",
                                        "data_id": 1,
                                        "code": "jjnjsd",
                                        "id": 68,
                                        "name": "hey",
                                        "unit": "kg"
                                    }
                                }
                            ],
                            "resources": [],
                            "child_resources": [],
                            "category_config": {},
                            "childs": {}
                        },
                        "8": {
                            "workstation": {
                                "id": 8,
                                "data_id": 1,
                                "name": "WS1.3",
                                "primary_flag": "NO",
                                "category_config": "{}",
                                "database": {
                                    "id": 1,
                                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                                    "key": "YLKS04NS2W",
                                    "regdate": "2023-11-18",
                                    "status": "Active"
                                }
                            },
                            "totals": {},
                            "recv_totals": {},
                            "wip_totals": {},
                            "reject_totals": {},
                            "material_issues": [],
                            "material_estimates": {},
                            "material_estimate_totals": {},
                            "material_estimate_totals_from_recv": {},
                            "material_issue_totals": {},
                            "material_return_totals": {},
                            "material_reject_totals": {},
                            "job_work_totals": 0,
                            "capacity_totals": 0,
                            "ws_date": "2024-06-01",
                            "jobs": [],
                            "resources": [],
                            "child_resources": [],
                            "category_config": {},
                            "childs": {}
                        },
                        "40": {
                            "workstation": {
                                "id": 40,
                                "data_id": 1,
                                "name": "ws1.4",
                                "primary_flag": null,
                                "category_config": "{}",
                                "database": {
                                    "id": 1,
                                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                                    "key": "YLKS04NS2W",
                                    "regdate": "2023-11-18",
                                    "status": "Active"
                                }
                            },
                            "totals": {},
                            "recv_totals": {},
                            "wip_totals": {},
                            "reject_totals": {},
                            "material_issues": [],
                            "material_estimates": {},
                            "material_estimate_totals": {},
                            "material_estimate_totals_from_recv": {},
                            "material_issue_totals": {},
                            "material_return_totals": {},
                            "material_reject_totals": {},
                            "job_work_totals": 0,
                            "capacity_totals": 0,
                            "ws_date": "2024-06-01",
                            "jobs": [],
                            "resources": [],
                            "child_resources": [],
                            "category_config": {},
                            "childs": {}
                        }
                    }
                },
                "9": {
                    "workstation": {
                        "id": 9,
                        "data_id": 1,
                        "name": "DHOOP SECTION",
                        "primary_flag": "NO",
                        "category_config": "{}",
                        "database": {
                            "id": 1,
                            "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                            "key": "YLKS04NS2W",
                            "regdate": "2023-11-18",
                            "status": "Active"
                        }
                    },
                    "totals": {},
                    "recv_totals": {},
                    "wip_totals": {},
                    "reject_totals": {},
                    "material_issues": [],
                    "material_estimates": {},
                    "material_estimate_totals": {},
                    "material_estimate_totals_from_recv": {},
                    "material_issue_totals": {},
                    "material_return_totals": {},
                    "material_reject_totals": {},
                    "job_work_totals": 0,
                    "capacity_totals": 320.0,
                    "ws_date": "2024-06-01",
                    "jobs": [],
                    "resources": [
                        {
                            "id": 58,
                            "resource_id": 2,
                            "time_lost": 0.0,
                            "contract_mode": "PAYROLL",
                            "time_allot": 8.0,
                            "workstation_id": 10,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "NA",
                                "name": "labor 2",
                                "salary": 20.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 2,
                                "gender": "Male",
                                "regdate": "2023-11-21",
                                "data_id": 1
                            }
                        },
                        {
                            "id": 60,
                            "resource_id": 2,
                            "time_lost": 0.0,
                            "contract_mode": "CONTRACT",
                            "time_allot": 8.0,
                            "workstation_id": 15,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "NA",
                                "name": "labor 2",
                                "salary": 20.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 2,
                                "gender": "Male",
                                "regdate": "2023-11-21",
                                "data_id": 1
                            }
                        }
                    ],
                    "child_resources": [
                        {
                            "id": 58,
                            "resource_id": 2,
                            "time_lost": 0.0,
                            "contract_mode": "PAYROLL",
                            "time_allot": 8.0,
                            "workstation_id": 10,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "NA",
                                "name": "labor 2",
                                "salary": 20.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 2,
                                "gender": "Male",
                                "regdate": "2023-11-21",
                                "data_id": 1
                            }
                        },
                        {
                            "id": 60,
                            "resource_id": 2,
                            "time_lost": 0.0,
                            "contract_mode": "CONTRACT",
                            "time_allot": 8.0,
                            "workstation_id": 15,
                            "date_allot": "2024-06-01",
                            "data_id": 1,
                            "resource": {
                                "code": "NA",
                                "name": "labor 2",
                                "salary": 20.0,
                                "status": "Active",
                                "contract_mode": "PAYROLL",
                                "id": 2,
                                "gender": "Male",
                                "regdate": "2023-11-21",
                                "data_id": 1
                            }
                        }
                    ],
                    "category_config": {},
                    "childs": {
                        "10": {
                            "workstation": {
                                "id": 10,
                                "data_id": 1,
                                "name": "DHOOP MACHINE 1",
                                "primary_flag": "NO",
                                "category_config": "{}",
                                "database": {
                                    "id": 1,
                                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                                    "key": "YLKS04NS2W",
                                    "regdate": "2023-11-18",
                                    "status": "Active"
                                }
                            },
                            "totals": {},
                            "recv_totals": {},
                            "wip_totals": {},
                            "reject_totals": {},
                            "material_issues": [],
                            "material_estimates": {},
                            "material_estimate_totals": {},
                            "material_estimate_totals_from_recv": {},
                            "material_issue_totals": {},
                            "material_return_totals": {},
                            "material_reject_totals": {},
                            "job_work_totals": 0,
                            "capacity_totals": 160.0,
                            "ws_date": "2024-06-01",
                            "jobs": [],
                            "resources": [
                                {
                                    "id": 58,
                                    "resource_id": 2,
                                    "time_lost": 0.0,
                                    "contract_mode": "PAYROLL",
                                    "time_allot": 8.0,
                                    "workstation_id": 10,
                                    "date_allot": "2024-06-01",
                                    "data_id": 1,
                                    "resource": {
                                        "code": "NA",
                                        "name": "labor 2",
                                        "salary": 20.0,
                                        "status": "Active",
                                        "contract_mode": "PAYROLL",
                                        "id": 2,
                                        "gender": "Male",
                                        "regdate": "2023-11-21",
                                        "data_id": 1
                                    }
                                }
                            ],
                            "child_resources": [
                                {
                                    "id": 58,
                                    "resource_id": 2,
                                    "time_lost": 0.0,
                                    "contract_mode": "PAYROLL",
                                    "time_allot": 8.0,
                                    "workstation_id": 10,
                                    "date_allot": "2024-06-01",
                                    "data_id": 1,
                                    "resource": {
                                        "code": "NA",
                                        "name": "labor 2",
                                        "salary": 20.0,
                                        "status": "Active",
                                        "contract_mode": "PAYROLL",
                                        "id": 2,
                                        "gender": "Male",
                                        "regdate": "2023-11-21",
                                        "data_id": 1
                                    }
                                }
                            ],
                            "category_config": {},
                            "childs": {}
                        },
                        "15": {
                            "workstation": {
                                "id": 15,
                                "data_id": 1,
                                "name": "MACHINE 2",
                                "primary_flag": "NO",
                                "category_config": "{}",
                                "database": {
                                    "id": 1,
                                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                                    "key": "YLKS04NS2W",
                                    "regdate": "2023-11-18",
                                    "status": "Active"
                                }
                            },
                            "totals": {},
                            "recv_totals": {},
                            "wip_totals": {},
                            "reject_totals": {},
                            "material_issues": [],
                            "material_estimates": {},
                            "material_estimate_totals": {},
                            "material_estimate_totals_from_recv": {},
                            "material_issue_totals": {},
                            "material_return_totals": {},
                            "material_reject_totals": {},
                            "job_work_totals": 0,
                            "capacity_totals": 160.0,
                            "ws_date": "2024-06-01",
                            "jobs": [],
                            "resources": [
                                {
                                    "id": 60,
                                    "resource_id": 2,
                                    "time_lost": 0.0,
                                    "contract_mode": "CONTRACT",
                                    "time_allot": 8.0,
                                    "workstation_id": 15,
                                    "date_allot": "2024-06-01",
                                    "data_id": 1,
                                    "resource": {
                                        "code": "NA",
                                        "name": "labor 2",
                                        "salary": 20.0,
                                        "status": "Active",
                                        "contract_mode": "PAYROLL",
                                        "id": 2,
                                        "gender": "Male",
                                        "regdate": "2023-11-21",
                                        "data_id": 1
                                    }
                                }
                            ],
                            "child_resources": [
                                {
                                    "id": 60,
                                    "resource_id": 2,
                                    "time_lost": 0.0,
                                    "contract_mode": "CONTRACT",
                                    "time_allot": 8.0,
                                    "workstation_id": 15,
                                    "date_allot": "2024-06-01",
                                    "data_id": 1,
                                    "resource": {
                                        "code": "NA",
                                        "name": "labor 2",
                                        "salary": 20.0,
                                        "status": "Active",
                                        "contract_mode": "PAYROLL",
                                        "id": 2,
                                        "gender": "Male",
                                        "regdate": "2023-11-21",
                                        "data_id": 1
                                    }
                                }
                            ],
                            "category_config": {},
                            "childs": {}
                        },
                        "16": {
                            "workstation": {
                                "id": 16,
                                "data_id": 1,
                                "name": "WS INDIVIDUAL",
                                "primary_flag": "NO",
                                "category_config": "{}",
                                "database": {
                                    "id": 1,
                                    "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                                    "key": "YLKS04NS2W",
                                    "regdate": "2023-11-18",
                                    "status": "Active"
                                }
                            },
                            "totals": {},
                            "recv_totals": {},
                            "wip_totals": {},
                            "reject_totals": {},
                            "material_issues": [],
                            "material_estimates": {},
                            "material_estimate_totals": {},
                            "material_estimate_totals_from_recv": {},
                            "material_issue_totals": {},
                            "material_return_totals": {},
                            "material_reject_totals": {},
                            "job_work_totals": 0,
                            "capacity_totals": 0,
                            "ws_date": "2024-06-01",
                            "jobs": [],
                            "resources": [],
                            "child_resources": [],
                            "category_config": {},
                            "childs": {}
                        }
                    }
                },
                "11": {
                    "workstation": {
                        "id": 11,
                        "data_id": 1,
                        "name": "CONE",
                        "primary_flag": "NO",
                        "category_config": "{}",
                        "database": {
                            "id": 1,
                            "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                            "key": "YLKS04NS2W",
                            "regdate": "2023-11-18",
                            "status": "Active"
                        }
                    },
                    "totals": {
                        "2": 25.0
                    },
                    "recv_totals": {
                        "2": 0
                    },
                    "wip_totals": {
                        "2": 0.0
                    },
                    "reject_totals": {
                        "2": 0.0
                    },
                    "material_issues": [
                        {
                            "workstation_id": 11,
                            "item_id": 4,
                            "issue_qty": 0.0,
                            "return_qty": 0.0,
                            "date_issue": "2024-06-01",
                            "inventory_id": 543,
                            "wip_flag": "NO",
                            "item_unit": "kgs",
                            "id": 105,
                            "reject_qty": 0.0,
                            "data_id": 1,
                            "reject_inventory_id": null,
                            "item": {
                                "rate": 11.0,
                                "regdate": "2023-11-20",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "rm1",
                                "id": 4,
                                "name": "rm item 1",
                                "unit": "kgs"
                            }
                        },
                        {
                            "workstation_id": 11,
                            "item_id": 3,
                            "issue_qty": 0.0,
                            "return_qty": 0.0,
                            "date_issue": "2024-06-01",
                            "inventory_id": 544,
                            "wip_flag": "NO",
                            "item_unit": "pcs",
                            "id": 106,
                            "reject_qty": 0.0,
                            "data_id": 1,
                            "reject_inventory_id": null,
                            "item": {
                                "rate": 0.0,
                                "regdate": "2023-11-20",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "sf1",
                                "id": 3,
                                "name": "Sabka Product",
                                "unit": "pcs"
                            }
                        },
                        {
                            "workstation_id": 11,
                            "item_id": 2,
                            "issue_qty": 0.0,
                            "return_qty": 0.0,
                            "date_issue": "2024-06-01",
                            "inventory_id": 545,
                            "wip_flag": "YES",
                            "item_unit": "dozend",
                            "id": 107,
                            "reject_qty": 0.0,
                            "data_id": 1,
                            "reject_inventory_id": null,
                            "item": {
                                "rate": 12.0,
                                "regdate": "2023-11-18",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "FG001",
                                "id": 2,
                                "name": "FG Item 1 bs",
                                "unit": "dozend"
                            }
                        }
                    ],
                    "material_estimates": {
                        "4": 13.125,
                        "3": 315.0,
                        "2": 25.0
                    },
                    "material_estimate_totals": {
                        "4": 13.125,
                        "3": 315.0,
                        "2": 25.0
                    },
                    "material_estimate_totals_from_recv": {
                        "4": 0.0,
                        "3": 0.0,
                        "2": 25.0
                    },
                    "material_issue_totals": {
                        "4": 0.0,
                        "3": 0.0,
                        "2": 0.0
                    },
                    "material_return_totals": {
                        "4": 0.0,
                        "3": 0.0,
                        "2": 0.0
                    },
                    "material_reject_totals": {
                        "4": 0.0,
                        "3": 0.0,
                        "2": 0.0
                    },
                    "job_work_totals": 300.0,
                    "capacity_totals": 0,
                    "ws_date": "2024-06-01",
                    "jobs": [
                        {
                            "date_allot": "2024-06-01",
                            "workstation_id": 11,
                            "data_id": 1,
                            "id": 160,
                            "inventory_id": 542,
                            "item_id": 2,
                            "wip_inventory_id": null,
                            "qty_allot": 25.0,
                            "reject_inventory_id": null,
                            "qty_recv": 0.0,
                            "qty_wip": 0.0,
                            "qty_reject": 0.0,
                            "item": {
                                "rate": 12.0,
                                "regdate": "2023-11-18",
                                "raw_flag": "NO",
                                "data_id": 1,
                                "code": "FG001",
                                "id": 2,
                                "name": "FG Item 1 bs",
                                "unit": "dozend"
                            }
                        }
                    ],
                    "resources": [],
                    "child_resources": [],
                    "category_config": {},
                    "childs": {}
                },
                "12": {
                    "workstation": {
                        "id": 12,
                        "data_id": 1,
                        "name": "INCENSE",
                        "primary_flag": "NO",
                        "category_config": "{}",
                        "database": {
                            "id": 1,
                            "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                            "key": "YLKS04NS2W",
                            "regdate": "2023-11-18",
                            "status": "Active"
                        }
                    },
                    "totals": {},
                    "recv_totals": {},
                    "wip_totals": {},
                    "reject_totals": {},
                    "material_issues": [],
                    "material_estimates": {},
                    "material_estimate_totals": {},
                    "material_estimate_totals_from_recv": {},
                    "material_issue_totals": {},
                    "material_return_totals": {},
                    "material_reject_totals": {},
                    "job_work_totals": 0,
                    "capacity_totals": 0,
                    "ws_date": "2024-06-01",
                    "jobs": [],
                    "resources": [],
                    "child_resources": [],
                    "category_config": {},
                    "childs": {}
                },
                "13": {
                    "workstation": {
                        "id": 13,
                        "data_id": 1,
                        "name": "FESTIVAL INCENSE",
                        "primary_flag": "NO",
                        "category_config": "{}",
                        "database": {
                            "id": 1,
                            "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                            "key": "YLKS04NS2W",
                            "regdate": "2023-11-18",
                            "status": "Active"
                        }
                    },
                    "totals": {},
                    "recv_totals": {},
                    "wip_totals": {},
                    "reject_totals": {},
                    "material_issues": [],
                    "material_estimates": {},
                    "material_estimate_totals": {},
                    "material_estimate_totals_from_recv": {},
                    "material_issue_totals": {},
                    "material_return_totals": {},
                    "material_reject_totals": {},
                    "job_work_totals": 0,
                    "capacity_totals": 0,
                    "ws_date": "2024-06-01",
                    "jobs": [],
                    "resources": [],
                    "child_resources": [],
                    "category_config": {},
                    "childs": {}
                },
                "14": {
                    "workstation": {
                        "id": 14,
                        "data_id": 1,
                        "name": "BACKFLOW CONE DEPARTMENT",
                        "primary_flag": "NO",
                        "category_config": "{}",
                        "database": {
                            "id": 1,
                            "name": "VEDANT AGRAWAL_J3VU4NZYT5",
                            "key": "YLKS04NS2W",
                            "regdate": "2023-11-18",
                            "status": "Active"
                        }
                    },
                    "totals": {},
                    "recv_totals": {},
                    "wip_totals": {},
                    "reject_totals": {},
                    "material_issues": [],
                    "material_estimates": {},
                    "material_estimate_totals": {},
                    "material_estimate_totals_from_recv": {},
                    "material_issue_totals": {},
                    "material_return_totals": {},
                    "material_reject_totals": {},
                    "job_work_totals": 0,
                    "capacity_totals": 0,
                    "ws_date": "2024-06-01",
                    "jobs": [],
                    "resources": [],
                    "child_resources": [],
                    "category_config": {},
                    "childs": {}
                }
            }
        }
    },
    "primary_ws_flag": false,
    "WORKSTATION_PATH ": [
        [
            1,
            "PRIMARY WORKSTATION"
        ]
    ],
    "segment": [
        "workstations"
    ],
    "categories": [
        [
            4,
            "RAW MATERIAL"
        ],
        [
            5,
            "PACKING MATERIAL"
        ],
        [
            3,
            "EXPORT"
        ],
        [
            6,
            "MACHINE"
        ],
        [
            7,
            "RAJASTHAN"
        ],
        [
            1,
            "QWERTY"
        ],
        [
            2,
            "SEMI-FINISHED"
        ],
        [
            15,
            "ABC"
        ]
    ],
    "CATEGORIES_MAP": {
        "4": "RAW MATERIAL",
        "5": "PACKING MATERIAL",
        "3": "EXPORT",
        "6": "MACHINE",
        "7": "RAJASTHAN",
        "1": "QWERTY",
        "2": "SEMI-FINISHED",
        "15": "ABC"
    }
}